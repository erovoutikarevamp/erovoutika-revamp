    ...
    <div className="space-y-10 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Manage industry partner records and assets.</p>
        </div>
        <Button
          onClick={openAdd}
        ...
        >
          <Plus className="w-3.5 h-3.5 mr-2" /> Add_Partner
        </Button>
      </div>

      {/* ── Stat Cards ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <DashboardStatCard title="Total_Partners" value={partners.length} icon={Building2} accent="#3b82f6" />
        <DashboardStatCard title="With_Logo"      value={withLogo}       icon={Building2} accent="#22c55e" />
        <DashboardStatCard title="With_Website"   value={withWebsite}    icon={Globe}     accent="#a855f7" />
      </div>
      ...
      <div className="relative p-5 bg-white dark:bg-slate-900/50 border border-gray-200 dark:border-white/[0.08] shadow-sm backdrop-blur-sm">
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Query registry by name or category..."
            value={searchQuery}
      ...
      <div className="flex items-center gap-4">
        <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-gray-400 dark:text-slate-500 shrink-0">Partner_Registry</p>
      ...
      {filteredPartners.length === 0 ? (
        <div className="bg-gray-50/50 dark:bg-white/[0.02] border border-dashed border-gray-200 dark:border-white/10 py-20 text-center">
          <p className="font-mono text-[10px] text-gray-400 dark:text-slate-600 uppercase tracking-widest">Null_Set: No industry partners found</p>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader className="bg-gray-50/50 dark:bg-white/[0.02] border-b dark:border-white/5">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="font-mono text-[10px] uppercase tracking-wider h-10">Partner_Record</TableHead>
                ...
                <TableRow key={p.id} className="group border-b dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{p.name}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    {p.category ? (
                  ...
                  <TableCell>
                    {p.website_url ? (
                      <a href={p.website_url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors font-mono">
        ...
        <AlertDialogContent className="rounded-none border-2 border-red-500/20 dark:bg-slate-900 dark:border-red-900/50">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs italic dark:text-slate-400">
              Warning: This action will purge the partner record from the central database.
